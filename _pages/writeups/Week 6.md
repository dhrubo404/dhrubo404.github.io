...
A knapsack auction an auction where the auctioneer has a limited capacity and must choose a subset of bids that maximize total value.
...
p_i = W^*_{-i} - \bigl(W^* - v_i\bigr), \qquad u_i = v_i - p_i .
...
I plan to study and see if we can incorporate a more approximate dynamic programming framework. Bidders and spectrum availability keep changing over time with allocation and payment decisions made online. Also try out an EV charging allocation using Knapsack and VCG to test it out. 
